...
#----------------------------
# User guide - Windows Subsystem for Linux version
#----------------------------
...
help()
{
    echo "generate-CDS-WSL.sh help page."
    echo
    echo "Generate CDS (coding protein sequences) from fasta (.fas) or gzipped fasta (.fas.gz) files."
    echo "generate-CDS-WSL.sh should be run with no arguments (or -h for help)."
    echo
    echo "Syntax: ./generate-CDS-WSL.sh"
    echo "        ./generate-CDS-WSL.sh -h"
    echo
}
...
while getopts h flag; do
    case "${flag}" in
        ...
        *) echo "Invalid argument. generate-CDS-WSL runs with no arguments (or -h for help)"
           exit 1
    esac
done
...
if ! dpkg-query -W -f='${Status}' transdecoder | grep "ok installed" &> /dev/null; then
...
# check for pip apt package
if ! dpkg-query -W -f='${Status}' python3-pip | grep "ok installed" &> /dev/null; then
...
for f in *.rmseq; do
    ...
    (
        ...
        # by redirection
        TransDecoder.LongOrfs -t "${f}" &> /dev/null
        TransDecoder.Predict -t "${f}" &> /dev/null
